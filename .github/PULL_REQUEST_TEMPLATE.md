...
- [ ] New agent
- [ ] Invariant change (requires maintainer sign-off)
- [ ] Schema change to `ENCODING.md` / `company.yaml`
- [ ] New framework or operator playbook
- [ ] New swipe-file post (annotated)
- [ ] Spec update
- [ ] Workflow / FSM
- [ ] Documentation
...
- [ ] Operator-name sweep clean (no real operator, agency, or client names anywhere)
- [ ] AI-attribution sweep clean (no `Co-Authored-By: Claude`, no `Generated with Claude Code`, no AI-author markers)
...
- [ ] `.gitignore` still protects credentials + per-client encoded data
- [ ] No `_private/` content, filled `company.yaml` values, or `.env` values leaked

## Skill-specific (if applicable)

- [ ] `SKILL.md` has all required body sections (Role, Decision Logic, Tacit Principles, Process, Output Format, Context Requirements, Verification Checklist, Important Rules, Cross-skill Routing)
